...
# ----------------------------------------
# INITIALISATION COMPLÈTE DE LA SESSION STATE
# ----------------------------------------
...
# Initialisation des variables de contrôle et de données
if 'control_type' not in st.session_state:
    st.session_state.control_type = "Standards CRM"  # Valeur par défaut
...
# Initialisation des variables de personnalisation
if 'graph_title' not in st.session_state:
    st.session_state.graph_title = "Contrôle Qualité - Standards CRM"
...
# Initialisation des valeurs pour l'analyse CRM
if 'reference_value' not in st.session_state:
    st.session_state.reference_value = 0.0
if 'reference_stddev' not in st.session_state:
    st.session_state.reference_stddev = 0.0
if 'tolerance_type' not in st.session_state:
    st.session_state.tolerance_type = "Pourcentage (%)"
if 'tolerance_percent' not in st.session_state:
    st.session_state.tolerance_percent = 10.0
if 'tolerance_stddev' not in st.session_state:
    st.session_state.tolerance_stddev = 2.0

# Initialisation des champs requis pour le premier type de contrôle par défaut
...
if st.session_state.tab == "Type de Contrôle":
    ...
    control_type = st.selectbox(
        ...
        index=["Standards CRM", "Blancs", "Duplicatas (nuage de points et régression)"].index(st.session_state.control_type),
        key="control_type_select"
    )
    ...
    with control_subtabs[0]:
        if control_type == "Standards CRM":
            ...
            with col1:
                ...
                default_ref_value = st.session_state.reference_value
                default_ref_stddev = st.session_state.reference_stddev
                ...
                reference_stddev = st.number_input(
                    ...
                    key="reference_stddev_input"
                )
                
                # Mettre à jour la session state
                st.session_state.reference_value = reference_value
                st.session_state.reference_stddev = reference_stddev
            
            with col2:
                tolerance_type = st.radio(
                    "Type de tolérance:",
                    ["Pourcentage (%)", "Multiple de l'écart-type"],
                    index=0 if st.session_state.tolerance_type == "Pourcentage (%)" else 1,
                    key="tolerance_type_radio"
                )
                
                # Mettre à jour la session state
                st.session_state.tolerance_type = tolerance_type
                
                if tolerance_type == "Pourcentage (%)":
                    default_tolerance = st.session_state.tolerance_percent
                    ...
                    tolerance_value = st.number_input(
                        ...
                        step=0.1,
                        key="tolerance_percent_input"
                    )
                    
                    # Mettre à jour la session state
                    st.session_state.tolerance_percent = tolerance_value
                else:
                    tolerance_value = st.number_input(
                        "Multiple de l'écart-type:",
                        min_value=0.0,
                        value=st.session_state.tolerance_stddev,
                        step=0.1,
                        key="tolerance_stddev_input"
                    )
                    
                    # Mettre à jour la session state
                    st.session_state.tolerance_stddev = tolerance_value
        ...
        st.session_state.graph_title = graph_title
    ...
    with control_subtabs[1]:
        ...
        if control_type == "Standards CRM":
            ...
            if st.button("Utiliser ces données d'exemple", key="use_crm_example"):
                ...
                # Mettre à jour immédiatement les valeurs de session state
    ...
    if import_method == "Téléchargement de fichier":
        ...
        # Récupérer le type de contrôle avec une valeur par défaut
        control_type = st.session_state.get('control_type', "Standards CRM")
    ...
    if st.session_state.data is None:
        ...
        # Vérifier que required_fields existe
        if 'required_fields' not in st.session_state or not st.session_state.required_fields:
            # Définir selon le type de contrôle actuel
            control_type = st.session_state.get('control_type', "Standards CRM")
            if control_type == "Standards CRM" or control_type == "Blancs":
                st.session_state.required_fields = {
                    "sample_id": "Identifiant de l'échantillon",
                    "measured_value": "Valeur mesurée"
                }
        ...
        if control_type == "Standards CRM":
            if st.button("Générer la Carte de Contrôle", key="generate_crm"):
                ...
                if analysis_data.empty:
                    ...
                    # Récupération des paramètres avec valeurs de secours
                    reference_value = st.session_state.get('reference_value', 0.0)
                    tolerance_type = st.session_state.get('tolerance_type', "Pourcentage (%)")
                    
                    if tolerance_type == "Pourcentage (%)":
                        tolerance_value = st.session_state.get('tolerance_percent', 10.0)
                    else:
                        tolerance_value = st.session_state.get('tolerance_stddev', 2.0)
                        
                    reference_stddev = st.session_state.get('reference_stddev', 0.0)
